...
def SRloss(pred, label, device):
    batch_size = label.size(0)
    height = label.size(1)
    width = label.size(2)
    matrix_torch = torch.from_numpy(mu).to(device)
    pred_distribution = torch.sum(F.log_softmax(pred,dim=1), axis=(2,3))/ (height*width)  # log space input for KLDivLoss
    
    label_distribution = torch.zeros([batch_size,8], dtype=torch.float32, requires_grad=False).to(device)
    for i in range (batch_size):

        label_set = label[i].unique(return_counts=True)[0]
        label_count = label[i].unique(return_counts=True)[1]
        for j in range(len(label_set)):
            label_distribution[i] += matrix_torch[label_set[j]] *  label_count[j]
        label_distribution[i] = label_distribution[i] / (height*width)

    kl_loss = nn.KLDivLoss(reduction="batchmean")
    loss = kl_loss(pred_distribution, label_distribution)
    return loss
...
def main():
    ...
    out_dir = osp.join(here,'results', 'LSR'+LABEL)
    ...
    path = osp.join(here,'results', 'LSR'+LABEL, 'checkpoints', 'iter' + str(args.resume) +'.pth')
    i_iter=0
    if args.resume: 
    ...
    while i_iter < max_iter:
        ...
        loss_sr = SRloss(pred1, label_s, device)
        loss = loss_seg + 0.005*loss_sr
