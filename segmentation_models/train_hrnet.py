...
def main():
   ...
   # save checkpoint direction
    ...
    out_dir = osp.join(here,'results', 'HRNet'+LABEL)
    ...
    path = osp.join(here,'results', 'HRNet'+LABEL, 'checkpoints', 'iter' + str(args.resume) +'.pth')
    i_iter=0
    if args.resume: 
    ...
    loss_fn =  CrossEntropy(ignore_label=args.num_classes)
